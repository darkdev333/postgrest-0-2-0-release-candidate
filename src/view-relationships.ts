...
export function qualifiedRelationKey(schema: string | undefined, table: string): string {
  return `${schema ?? ''}\u0000${table}`
}

function dependencyMatches(
  dependency: ViewKeyDependency,
  tableSchema: string | undefined,
  tableName: string,
  constraintName: string,
  type: ViewKeyDependencyType,
): boolean {
  if (dependency.tableName !== tableName || dependency.constraintName !== constraintName || dependency.type !== type) return false
...
export function expandViewColumnDependencies(columns: ViewColumnDependency[]): RelationshipColumnPair[][] {
  let combinations: RelationshipColumnPair[][] = [[]]
  for (const column of columns) {
    const next: RelationshipColumnPair[][] = []
    for (const combination of combinations) {
      for (const viewColumn of column.viewColumns) next.push([...combination, { source: column.tableColumn, target: viewColumn }])
    }
    combinations = next
  }
  return combinations
...
export function deriveViewRelationships(directRelationships: RelationshipInfo[], dependencies: ViewKeyDependency[]): RelationshipInfo[] {
  const derived: RelationshipInfo[] = []
  for (const relationship of directRelationships) {
    if (relationship.computed) continue
    if (relationship.cardinality !== 'many-to-one' && relationship.cardinality !== 'one-to-one') continue

    const viewTableDeps = dependencies.filter(dependency => dependencyMatches(
      dependency,
    ...
    const tableViewDeps = dependencies.filter(dependency => dependencyMatches(
      dependency,
...
export function viewPrimaryKeys(dependencies: ViewKeyDependency[]): Map<string, string[]> {
  const result = new Map<string, string[]>()
  for (const dependency of dependencies) {
    if (dependency.type !== 'p') continue
    const columns = dependency.columns.map(column => column.viewColumns[0]).filter((column): column is string => Boolean(column))
    ...
    if (!result.has(dependency.viewName)) result.set(dependency.viewName, [...columns])
  }
  return result
}
