import { PostgrestParser, type Filter, type OrderClause } from './parser.js'
import { parseReadLogicFilter } from './read-logic.js'
import type { ReadLogicTerm, ReadOrderClause, ReadPlan, RelatedOrderClause } from './read-plan.js'
...
export class ReadQueryPathError extends Error {
  ...
  constructor(resource: string, hintText?: string, detailsText: string | null = null) {
...
function clonePlan(plan: ReadPlan): ReadPlan {
  return {
    ...plan,
    fields: [...plan.fields],
...
function resolvePath(root: ReadPlan, path: string[]): ReadPlan {
  let current = root
  for (const segment of path) {
    const byOutput = current.embeds.find(embed => embed.outputName === segment)
    if (byOutput) { current = byOutput.plan; continue }

    const aliasedTarget = current.embeds.find(embed => embed.relation === segment && embed.outputName !== embed.relation)
    if (aliasedTarget) {
      throw new ReadQueryPathError(
        segment,
        `Use '${aliasedTarget.outputName}' instead of '${segment}' to reference this embedded resource.`,
...
function assignFilter(target: ReadPlan, filter: Filter | null): void {
  if (!filter) return
  ;(target.filters ??= []).push(filter)
}

function splitOrderTerms(value: string): string[] {
  const parts: string[] = []
  let current = ''
  let depth = 0
...
function parseRelatedOrder(term: string): RelatedOrderClause | null {
  ...
  const result: RelatedOrderClause = { relation, column, direction }
  if (match[4]) result.nullsFirst = match[4].toLowerCase() === 'nullsfirst'
  return result
}

function parseReadOrder(target: ReadPlan, value: string, parser: PostgrestParser): ReadOrderClause[] {
  const result: ReadOrderClause[] = []
  for (const term of splitOrderTerms(value)) {
    const related = parseRelatedOrder(term)
    if (!related) {
      result.push(...parser.parseOrder(term) as OrderClause[])
      continue
    }
    const embed = target.embeds.find(candidate => candidate.outputName === related.relation)
    if (!embed) throw new ReadQueryPathError(related.relation)
    if (!isToOneRelationship(embed.relationship)) throw new RelatedOrderError(target.table, related.relation)
    result.push(related)
  }
  return result
}

function assignOrder(target: ReadPlan, value: string, parser: PostgrestParser): void {
  target.order = parseReadOrder(target, value, parser)
}

function toReadLogicTerm(target: ReadPlan, filter: Filter): ReadLogicTerm {
  if (filter.operator === 'and' || filter.operator === 'or') {
    return {
      kind: 'logic',
      operator: filter.operator,
      negate: filter.negate === true,
      terms: (filter.value as Filter[]).map(child => toReadLogicTerm(target, child)),
    }
  }
...
function assignLogic(target: ReadPlan, filter: Filter | null): void {
  if (!filter) return
  ;(target.logic ??= []).push(toReadLogicTerm(target, filter))
}

function assignEmbedNullFilter(target: ReadPlan, resource: string, filter: Filter | null): boolean {
  if (!filter || filter.operator !== 'is' || filter.value !== null) return false
...
export function applyReadQueryParams(
  plan: ReadPlan,
  ...
  for (const [key, value] of searchParams.entries()) {
    ...
    if (leaf === 'limit' || leaf === 'offset') {
      ...
      if (numeric !== undefined) resolvePath(result, parts.slice(0, -1))[leaf] = numeric
      continue
    }
    if (leaf === 'and' || leaf === 'or') {
    ...
    if (!assignEmbedNullFilter(target, leaf, filter)) assignFilter(target, filter)
  }
