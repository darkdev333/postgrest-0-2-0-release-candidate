...
export type RangeRequest =
  | { kind: 'absent' }
  | { kind: 'valid'; offset: number; limit?: number }
  | { kind: 'invalid'; details: string };

/** Parse Range using PostgREST's item-range semantics, including open-ended ranges. */
export function parseRangeRequest(header: string | null | undefined): RangeRequest {
...
export function negativeLimitDetails(searchParams: URLSearchParams): string | null {
  const raw = searchParams.get('limit');
  if (raw === null) return null;
  if (/^-\d+$/.test(raw)) return 'Limit should be greater than or equal to zero.';
  return null;
}

/** Details string used when a known total cannot satisfy a non-zero offset. */
export function offsideOffsetDetails(offset: number, totalCount: number): string {
...
export function readStatus(offset: number, rowCount: number, totalCount?: number): 200 | 206 {
  if (totalCount === undefined || rowCount === 0) return 200;
