import type { ViewColumnDependency, ViewKeyDependency, ViewKeyDependencyType } from './view-relationships.js'
...
function decodeColumns(value: unknown): ViewColumnDependency[] {
  let decoded: unknown = value
  if (typeof decoded === 'string') {
    try { decoded = JSON.parse(decoded) } catch { return [] }
  }
  if (!Array.isArray(decoded)) return []
  const result: ViewColumnDependency[] = []
  for (const item of decoded) {
    ...
    if (item && typeof item === 'object') {
      ...
      if (tableColumn != null) result.push({ tableColumn: String(tableColumn), viewColumns: stringArray(viewColumns) })
    }
  }
  return result
}

function dependencyType(value: unknown): ViewKeyDependencyType | null {
  return value === 'p' || value === 'f' || value === 'f_ref' ? value : null
}

export function viewDependenciesFromCatalogRows(rows: Record<string, unknown>[]): ViewKeyDependency[] {
  const result: ViewKeyDependency[] = []
