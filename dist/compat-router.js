...
import { createPostgRESTRouter as createBaseRouter } from './router.js';
...
import { parsePreferHeader, responsePreferences, setResponseHeaders, setCORSHeaders } from './headers.js';
...
function singular(c, rows, status) {
    if (rows.length !== 1)
        return c.json(singularCardinalityError(rows.length), 406);
    c.header('Content-Type', SINGULAR_MEDIA_TYPE);
    return c.json(rows[0], status);
}
function rangeFailure(c, details, totalCount, head = false) {
    c.header('Content-Type', 'application/json');
    c.header('Range-Unit', 'items');
    if (totalCount !== undefined)
        c.header('Content-Range', `*/${totalCount}`);
    return head ? c.body(null, 416) : c.json(requestedRangeNotSatisfiable(details), 416);
}
function clampRootRange(plan, defaultLimit, maxLimit) {
...
export function createPostgRESTRouter(sql, options = {}) {
    ...
    const tableCacheFor = (activeSchema) => {
    ...
    const relationshipCacheFor = (activeSchema) => {
    ...
    async function inTx(c, activeSchema, prefer, callback) {
        const baseContext = transactionContext ? await transactionContext(c.req.raw, activeSchema) : { schema: activeSchema };
        const context = { ...baseContext, schema: baseContext.schema ?? activeSchema };
        if (dbTxEnd && prefer.tx)
            context.transactionEnd = prefer.tx;
        return withOptionalTransaction(sql, callback, context);
    }
    function corsConfig(origin) {
        const config = {};
        if (corsCredentials !== undefined)
            config.credentials = corsCredentials;
        if (corsMethods)
            config.methods = corsMethods;
        if (corsAllowHeaders)
            config.allowHeaders = corsAllowHeaders;
        if (corsExposeHeaders)
            config.exposeHeaders = corsExposeHeaders;
        if (corsMaxAge !== undefined)
            config.maxAge = corsMaxAge;
        if (corsOrigins === '*')
            return { ...config, origin: '*' };
        if (corsOrigins instanceof RegExp)
            return { ...config, originPattern: corsOrigins, ...(origin !== undefined && { requestOrigin: origin }) };
        if (typeof corsOrigins === 'string')
            return origin && origin !== corsOrigins ? undefined : { ...config, origin: corsOrigins };
        if (Array.isArray(corsOrigins) && corsOrigins.length && origin !== undefined)
            return { ...config, allowedOrigins: corsOrigins, requestOrigin: origin };
        return undefined;
    }
    if (cors)
        app.use('*', async (c, next) => {
            ...
            if (origin && config)
                setCORSHeaders(c.res.headers, config);
        });
    app.on(['GET', 'HEAD'], '/:table', async (c) => {
        const url = new URL(c.req.url);
        const selectRaw = url.searchParams.get('select');
        if (!selectRaw)
            return base.fetch(c.req.raw);
        let selectTree;
        try {
            selectTree = parsePostgRESTSelect(selectRaw);
        }
        catch {
            return base.fetch(c.req.raw);
        }
        if (!hasEmbed(selectTree))
            return base.fetch(c.req.raw);
        ...
        if (profile.error)
            return isHead ? c.body(null, 406) : c.json(profile.error, 406);
        const activeSchema = profile.schema;
        if (!validateTable(table))
            return isHead ? c.body(null, 400) : c.json({ error: 'Invalid table name' }, 400);
        const tableCache = tableCacheFor(activeSchema);
        if (!(await tableCache.hasTable(table)))
            return isHead ? c.body(null, 404) : c.json({ error: `Table "${table}" not found` }, 404);
        const prefer = parsePreferHeader(c.req.header('Prefer'));
        if (prefer.handling === 'strict' && prefer.invalid?.length)
            return isHead ? c.body(null, 400) : c.json(invalidPreferences(prefer.invalid), 400);
        if (!dbTxEnd)
            delete prefer.tx;
        else if (prefer.tx && !hasTransactionCapability(sql))
            return isHead ? c.body(null, 501) : c.json({ code: 'PGRST501', details: null, hint: 'Provide SQLExecutor.transaction(callback, context) for rollback-capable request execution.', message: 'tx preference requires a transactional SQL executor' }, 501);
        try {
            ...
            plan = await expandSpreadStars(plan, async (targetTable) => {
                ...
                if (!target)
                    throw new Error(`Table "${targetTable}" not found while expanding spread`);
            ...
            if (reqRange.kind === 'valid') {
                ...
                if (reqRange.limit !== undefined)
                    plan.limit = Math.min(reqRange.limit, maxLimit);
            }
            ...
            if (error instanceof RelationshipResolutionError) {
                if (error.code === 'PGRST201')
                    return isHead ? c.body(null, 300) : c.json(ambiguousRelationship(error.sourceTable, error.targetTable, error.candidates), 300);
