 ...
 */
async function importHmacKey(secret, hashAlgorithm) {
    const encoder = new TextEncoder();
    return crypto.subtle.importKey('raw', encoder.encode(secret), { name: 'HMAC', hash: hashAlgorithm }, false, ['verify']);
}
/**
...
async function importRsaPublicKey(pemKey) {
    ...
    return crypto.subtle.importKey('spki', derBuffer, { name: 'RSASSA-PKCS1-v1_5', hash: 'SHA-256' }, false, ['verify']);
}
/**
 ...
 */
async function verifySignature(token, algorithm, key) {
    try {
        const parts = token.split('.');
        if (parts.length !== JWT_PART_COUNT) {
            return false;
        ...
        const encoder = new TextEncoder();
        const messageBuffer = encoder.encode(`${headerB64}.${payloadB64}`).buffer;
        ...
        if (algorithm === 'RS256') {
            ...
            return await crypto.subtle.verify('RSASSA-PKCS1-v1_5', cryptoKey, signatureBuffer, messageBuffer);
        }
        return false;
    }
    catch {
...
export function createAuthMiddleware(config) {
    return async (c, next) => {
        ...
        if (authHeader?.startsWith(BEARER_PREFIX)) {
            ...
            if (!token) {
                return handleAuthError(new Error('Empty Bearer token'), c, config, false);
            }
            if (config.jwt) {
                ...
                if (result.valid && result.payload) {
                    const authResult = {
                        ...
                        role: result.payload.role || DEFAULT_JWT_ROLE,
                        method: 'jwt',
                ...
                return handleAuthError(new Error(result.error || 'Invalid token'), c, config, false);
            }
        }
        ...
        if (config.apiKey) {
            ...
            if (apiKey) {
                ...
                return handleAuthError(new Error(result.error || 'Invalid API key'), c, config, false);
            }
        }
        ...
        return handleAuthError(new Error('No authentication provided - authentication required'), c, config, true);
    };
}
/**
 ...
 */
function handleAuthError(error, c, config, isNoAuth) {
    if (config.onError) {
