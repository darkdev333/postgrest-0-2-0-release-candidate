import { deriveViewRelationships, qualifiedRelationKey, viewPrimaryKeys } from './view-relationships.js';
function subset(values, container) {
...
function inverse(relationship) {
    if (relationship.computed || relationship.cardinality === 'many-to-many')
        return null;
...
export function addInverseRelationships(directRelationships) {
...
function junctionFor(left, right) {
    return {
...
function primaryKeyFor(primaryKeys, schema, table) {
    return primaryKeys.get(qualifiedRelationKey(schema, table)) ?? primaryKeys.get(table) ?? [];
}
export function discoverManyToManyRelationships(directRelationships, primaryKeys) {
...
export function assembleViewAwareRelationships(baseDirectRelationships, basePrimaryKeys, dependencies) {
    ...
    for (const [view, columns] of viewPrimaryKeys(dependencies))
        primaryKeys.set(view, columns);
    const m2m = discoverManyToManyRelationships(direct, primaryKeys);
    return addInverseRelationships([...direct, ...m2m]);
}
//# sourceMappingURL=relationship-assembly.js.map